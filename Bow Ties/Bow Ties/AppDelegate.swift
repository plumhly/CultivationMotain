...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        viewController.managedContex = contex;
        
        /*
        let contex = persistentContainer.viewContext
        
        let bowtie = NSEntityDescription.insertNewObject(forEntityName: "Bowtie", into: contex) as! Bowtie
        bowtie.name = "My bow tie"
        bowtie.lastWorn = NSDate()
        do {
            try contex.save()
        } catch let error as NSError {
            print("Saving error:\(error.description)")
        }
        ...
        do {
            ...
            let results =
            try contex.execute(request) as! NSAsynchronousFetchResult<NSFetchRequestResult>
            let ties = results.finalResult as! [Bowtie]
            let sample = ties[0]
